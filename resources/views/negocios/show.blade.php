...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    @if(session('error'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 8000)"
            ...
            <svg class="w-5 h-5 text-red-500 mt-0.5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm-1-5a1 1 0 112 0 1 1 0 01-2 0zm1-8a.75.75 0 01.75.75v4.5a.75.75 0 01-1.5 0v-4.5A.75.75 0 0110 5z" clip-rule="evenodd" />
            </svg>
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="flex-1 min-w-0">
            ...
            {{-- Compartir --}}
            <div class="flex items-center gap-2 mb-6" x-data="{ copied: false }">
                <span class="text-xs text-gray-400 font-medium mr-1">Compartir</span>
                {{-- WhatsApp --}}
                <a href="https://wa.me/?text={{ urlencode($lugar->nombre . ' — Guía Local ' . route('negocios.show', $lugar)) }}"
                ...
                {{-- Facebook --}}
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('negocios.show', $lugar)) }}"
                   target="_blank" rel="noopener"
                ...
                {{-- X (Twitter) --}}
                <a href="https://twitter.com/intent/tweet?text={{ urlencode($lugar->nombre . ' — Guía Local') }}&url={{ urlencode(route('negocios.show', $lugar)) }}"
                   target="_blank" rel="noopener"
                   class="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors"
                   aria-label="Compartir en X">
                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                </a>
                {{-- Copiar link --}}
                <button @click="navigator.clipboard.writeText('{{ route('negocios.show', $lugar) }}'); copied = true; setTimeout(() => copied = false, 2000)"
